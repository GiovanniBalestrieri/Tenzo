/*****************************************************************
 *            Arduino & L3G4200D gyro & KALMAN & CMPS10 Compass  *
...
float alphaW = 0.8; //0.15; // Exp(-2PI*Fcutoff/SamplingTime)
//
...
float alphaA_xy = 0.6; //0.02;  //use 'a' to increse and 'z' to decrease
float alphaA_z  = 0.6; //0.01;  //use 'h' to increase 'n' to decrease
...
// threshold gyro 
#define gyrothresholdLow 900
#define gyrothresholdMid 1300
...
{
   bearingCmps =  my_compass.bearing();
   int pitchCmps = my_compass.pitch();
   int rollCmps = my_compass.roll();
    ...
    *  Yaw   --  Z
    */
   float angPosFilter[] = {rollCmps,pitchCmps,bearingCmps};
   //  Filter data  
   if (filterAng == 1)
   {
   ...
   {
     displayValues(bearingCmps,pitchCmps,rollCmps);
   }
   //delay(100); 
}
...
void control()
 ...
 if (pid)
 {
 double rollError = abs(Setpoint-roll);
 double pitchError = abs(Setpoint-pitch);
...
void estimateAngle()
{
  ...
  yAngle = -yAngle;
  zAngle = kalmanZ.getAngle(angleZAcc,Wfilter[2],(double)(micros() - timerMu)) + kalmanZOffset;
