    ...
    function popupCallback(src,~)
        ...
        if val == 2
            ...
            %             %Plot the X magnitude
                        ...
                        plot(h1,index,gxdata,'r','LineWidth',2);%,'MarkerEdgeColor','k','MarkerFaceColor','g','MarkerSize',5);
            %             xlabel('Time');
            %             ylabel('Wx');
            ...
            %             title('Y angular velocity in deg/s');
                        plot(h2,index,gydata,'b','LineWidth',2);%,'MarkerEdgeColor','k','MarkerFaceColor','g','MarkerSize',5);
            %             xlabel('Time');
            %             ylabel('Wy Acc');
            %             axis([1 buf_len -80 80]);
            ...
            %             %hold on;
                        plot(h3,index,gzdata,'g','LineWidth',2);%,'MarkerEdgeColor','k','MarkerFaceColor','g','MarkerSize',5);
            %             axis([1 buf_len -80 80]);
            %             xlabel('Time');
            %             ylabel('Wz Acc');
                    end
    ...
    function connection(~,~)
        ...
        if get(connect,'Value') == 1
            ...
            if (strcmp(deblank(ack), 'K') == 1)
                ...
                tenzo = true;
                t = timer('ExecutionMode','FixedRate','Period',0.1,'TimerFcn',{@storeDataFromSerial});
                start(t);
                fprintf(xbee,'M') ; 
