...
function selcbkControl(source,eventdata)
    ...
    function connection(~,~)
        ...
        if get(connect,'Value') == 1
            ...
            oldSerial = instrfind('Port', '/dev/ttyUSB0'); 
            ...
            if (~isempty(oldSerial))  
                disp('WARNING:  /dev/ttyUSB0 in use.  Closing.')
                delete(oldSerial)
            end
            ...
            xbee = serial('/dev/ttyUSB0','baudrate',9600,'terminator','CR','tag','Quad');
