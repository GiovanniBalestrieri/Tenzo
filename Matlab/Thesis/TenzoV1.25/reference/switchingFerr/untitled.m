...
global A1 B1 C1 A2 B2 C2 r Tf nsample os_trsold x0 x0c w1num w1den n m x0t range resolution num den j_opt y_rif dy_rif g k a b
...
Ki=X(2)
Jval
[t,x_2]=ode45(@phase2,[0 Tf],[0, x0]);
y_2=x_2*[0;1;0];
figure(19)
plot(t,y_2',t,1-exp(-g*t));
legend('y','reference');
x0=[0 0 0];
C1=[0 1 0];
C2=[0 1 0];
...
beq = Bo.*0;
lb = [0 0 0 0];
ub = [1000 1000 1000 Tf];
ci=[0.1 0.1 0.1 0.1];
[X,Jval] = fmincon('d_parameters_ts',ci,Ao,Bo,Aeq,beq,lb,ub,@nonlcon_2,options);
Kp=X(1)
Ki_1=X(2)
Ki_2=X(3)
t1=X(4)
...
[tsim2,x2]=ode45(@phase2,[0 Tf],[xt1]);
f2=x2*C2';
...
hold on;
plot(t,y_2','r');
plot(tsi,[f1',f2'],tsi,rf,[t1,t1],[0,1.5],'b--');
legend('optimized PI','switching PI','$\overline{r}(t)$');
xlabel('time');
grid on;
plotParam;
...
[tsim2,x2]=ode45(@phase2,[0 Tf],[x0c xt1]);
f2=x2*C2';
[tsimo,x2]=ode45(@phase2,[0 Tf],[x0c x0]);
f2o=x2*C2';
[tsimu,x1]=ode45(@phase1,[0 Tf],x0);
f1u=x1*C1';
