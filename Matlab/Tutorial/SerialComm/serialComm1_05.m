...
% end
    try
        while (get(xbee, 'BytesAvailable')~=0)
            ...
            [Roll, theta, Pitch, pitch, Yaw, yaw, ComplRoll, cr, ComplPitch, cp, KalmanRoll, kr, KalmanPitch, kp, OmegaX, wx, OmegaY, wy, OmegaZ, wz, AccX, ax, AccY, ay, AccZ, az] = strread(sentence,'%s%f%s%f%s%f%s%f%s%f%s%f%s%f%s%f%s%f%s%f%s%f%s%f%s%f',1,'delimiter',',');
            ...
            %% Plotting angular error Complementary Filter
            % Figure options
            figure(4);
            % Update the rolling plot. Append the new reading to the end of the
            % rolling plot data
            ECXdata = [ ECXdata(2:end) ; (cr - theta) ];
            ECYdata = [ ECYdata(2:end) ; (cp - pitch) ];
            %Plot the X magnitude
            subplot(3,1,1);
            title('Comp. Roll est Error');
            plot(index,axdata,'r','LineWidth',2);%,'MarkerEdgeColor','k','MarkerFaceColor','g','MarkerSize',5);
            xlabel('Time');
            ylabel('Roll - Error Comp Est');
            axis([1 buf_len -2.3 2.3]);
            %hold on;
            subplot(3,1,2);
            title('Comp. Pitch est Error');
            plot(index,aydata,'b','LineWidth',2);%,'MarkerEdgeColor','k','MarkerFaceColor','g','MarkerSize',5);
            xlabel('Time');
            ylabel('Pitch - Error Comp Est');
%             axis([1 buf_len -2.3 2.3]);
%             subplot(3,1,3);
%             title('Z Axis Acceleration in G');
%             %hold on;
