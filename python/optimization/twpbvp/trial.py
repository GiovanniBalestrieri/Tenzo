...
def function(a , T):
    return numpy.array([T[1] ,        # evaluate dT1/dA
                       T[0]*arg1 - (T[1]**2/(1+T[1]**2))*arg2])  # T[0]*arg1 -arg2*T[1]**2/(1+T[1]**2)**(1/2)])#T[0]*arg1 - (T[1]**2/(1+T[1]**2))*arg2])    # evaluate dT2/dA
...
"""
import pylab
pylab.plot(A, T[0,:],'g',label="x(t)")
pylab.plot(A, T[1,:],'r',label="v(t)")
pylab.grid()
pylab.show()
